  ...
  function onScan(err, data) {
    if (err) {
      ...
      data.Items.forEach(function (flashcard) {
        console.log(flashcard.front + ": ", flashcard.behind);
...
app.post("/flashCard", async (req, res) => {
  ...
  if (req.body.front == null && req.body.behind == null) {
    res
      .status(400)
      .json({ info: "variable `front` or `behind` not included in json " });
    return;
  }
  var id = uuid.v4();
  var params = {
  ...
  if (itemAdded) {
    res.status(200).json({
      info: `flashCard ${req.body.front},${req.body.behind} added`,
      id: id,
    });
  } else {
...
app.put("/flashCard", async (req, res) => {
  if (
    (req.body.id == null, req.body.front == null && req.body.behind == null)
  ) {
    res.status(400).json({
      info: "variable `id`, `front` or `behind` not included in json ",
    });
    return;
  }
  ...
  if (itemUpdated) {
    res
      .status(200)
      .json({ info: `flashCard ${req.body.id} modified`, id: req.body.id });
  } else {
...
app.delete("/flashCard", async (req, res) => {
  ...
  if (isDeleted) {
    res.status(200).json({
      info: `flashCard ${req.query.id} has been removed`,
      id: req.query.id,
    });
  } else {
